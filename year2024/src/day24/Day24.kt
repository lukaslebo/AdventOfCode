...
private fun Mainboard.swap(swaps: Iterable<Pair<String, String>>): Mainboard {
    val wireByName = gates.map { it.outputWire }.associateBy { it.name }
    val swappedGates = gates.map { gate ->
        val swap = swaps.find { gate.outputWire.name == it.first || gate.outputWire.name == it.second }
        if (swap?.first == gate.outputWire.name) gate.copy(outputWire = wireByName.getValue(swap.second))
        else if (swap?.second == gate.outputWire.name) gate.copy(outputWire = wireByName.getValue(swap.first))
        else gate
    }
    return copy(gates = swappedGates)
}
